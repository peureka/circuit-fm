...
function createCircuitClient({ baseUrl, token, fetchImpl = global.fetch }) {
  ...
  return {
    ...
    // List the organiser's campaigns (cursor-paginated, newest first).
    // Used by api/broadcast.js to count recent sends for rate limiting.
    // Each item carries { id, name, subject, status, sentAt, sendAt,
    // createdAt, ... } — see Circuit's GET handler for the full shape.
    async listCampaigns({ limit, cursor } = {}) {
      const params = new URLSearchParams();
      if (limit) params.set("limit", String(limit));
      if (cursor) params.set("cursor", cursor);
      const qs = params.toString() ? `?${params.toString()}` : "";
      return request(`/api/organiser/v1/campaigns${qs}`, { method: "GET" });
    },
  };
}
